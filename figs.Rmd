...
mylabs <- as_labeller(
  c(
    ...
    "phylop_dmel" = paste(expression(italic("D. mel"))),  # paste("foo~", expression(italic("bar")))
    "phylop_dsim" = paste(expression(italic("D. sim")))
  ),
  label_parsed
)

scores_m <- select(scores, -browser_phylop_allbr) %>%
  gather(branch, phylop, starts_with("phylop_d"))

subtreescores <- ggplot(scores_m, aes(x = phylop, fill = branch), color = "grey20") +
...
subtreescores
ggsave(plot = subtreescores, "figs/subtree-phylop-scores-hist-3.7k.pdf")
```
...
```{r, tab_cap = "PhyloP scores"}
scores %>%
  group_by(isConsEl) %>%
  summarize_each(funs(mean, sem), starts_with('browser_phylop_allbr')) %>%
  kbl_rename() %>%
  knitr::kable()
```
...
scores %>%
  group_by(isConsEl) %>%
  summarize_each(funs(mean, sem), starts_with('phylop_d')) %>%
  kbl_rename() %>%
  knitr::kable()
```
...
```{bash}
column -t reg_3.7/phylop/allbr-phylop27-wholeregion-con.tsv
```
